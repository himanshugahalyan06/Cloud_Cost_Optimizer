...
def create_dashboard_plots(history, episode_info, title=""):
    ...
    fig = make_subplots(
        rows=3, cols=2,
        subplot_titles=(
            "📈 Traffic vs Served Requests", "🖥️ Active Servers",
            "📊 CPU Load", "⏱️ Latency (ms)",
            "❌ Dropped Requests", "💰 Cumulative Cost"
        ),
    ...
    # Traffic vs Served
    fig.add_trace(go.Scatter(x=steps, y=history["traffic"], name="Incoming",
                             line=dict(color=color_traffic, width=2)), row=1, col=1)
    fig.add_trace(go.Scatter(x=steps, y=history["served"], name="Served",
                             line=dict(color=color_served, width=2, dash="dash")), row=1, col=1)

    # Active Servers
    fig.add_trace(go.Scatter(x=steps, y=history["servers"], name="Servers",
                             fill="tozeroy", fillcolor="rgba(118,75,162,0.3)",
                             line=dict(color=color_servers, width=2)), row=1, col=2)

    # CPU Load
    fig.add_trace(go.Scatter(x=steps, y=history["cpu_load"], name="CPU",
                             fill="tozeroy", fillcolor="rgba(247,151,30,0.2)",
                             line=dict(color=color_cpu, width=2)), row=2, col=1)
    ...
    # Latency
    fig.add_trace(go.Scatter(x=steps, y=history["latency"], name="Latency",
                             line=dict(color=color_latency, width=2)), row=2, col=2)
    fig.add_hline(y=200, line_dash="dash", line_color="red", opacity=0.7,
                  annotation_text="SLA: 200ms", row=2, col=2)

    # Dropped
    fig.add_trace(go.Bar(x=steps, y=history["dropped"], name="Dropped",
                         marker_color=color_dropped, opacity=0.7), row=3, col=1)

    # Cumulative Cost
    cum_cost = np.cumsum(history["cost"])
    fig.add_trace(go.Scatter(x=steps, y=cum_cost, name="Cost",
                             fill="tozeroy", fillcolor="rgba(150,201,61,0.2)",
                             line=dict(color=color_cost, width=2)), row=3, col=2)
...
if run_button:
    ...
    fig = create_dashboard_plots(
        ...
        title=f"Simulation: {agent_type} on {task} (Score: {score:.3f})"
    )
