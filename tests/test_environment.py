...
from env.server_model import ServerFleet, ServerConfig
from utils.reward import compute_reward, RewardConfig
from tasks.graders import grade_episode, GradingConfig, load_grading_config
...
class TestPydanticModels:
    ...
    def test_observation_to_flat_array(self):
        obs = Observation(
            timestep=5, incoming_requests=100.0, active_servers=3,
            warming_up_servers=1, cpu_load=0.5, latency_ms=20.0,
            dropped_requests=0.0, served_requests=100.0,
            cost_so_far=1.5, traffic_trend=2.0, time_of_day=0.3,
        )
    ...
    def test_observation_to_prompt(self):
        obs = Observation(
            timestep=10, incoming_requests=150.0, active_servers=4,
            warming_up_servers=1, cpu_load=0.72, latency_ms=45.3,
            dropped_requests=5.0, served_requests=145.0,
            cost_so_far=4.2, traffic_trend=-3.0, time_of_day=0.75,
        )
...
class TestGraders:
    ...
    def _make_episode_info(self, **kwargs) -> EpisodeInfo:
        defaults = dict(
            total_steps=100, total_reward=50.0, total_requests=10000.0,
            total_served=9900.0, total_dropped=100.0, total_cost=50.0,
            avg_latency=30.0, max_latency=150.0, avg_cpu_load=0.6,
            sla_compliance_rate=0.95, avg_active_servers=5.0, peak_servers=8,
        )
    ...
    def test_perfect_episode_scores_high(self):
        info = self._make_episode_info(
            total_served=10000.0, total_dropped=0.0, total_cost=40.0,
            avg_latency=15.0, max_latency=50.0, sla_compliance_rate=1.0,
        )
    ...
    def test_terrible_episode_scores_low(self):
        info = self._make_episode_info(
            total_served=5000.0, total_dropped=5000.0, total_cost=200.0,
            avg_latency=500.0, max_latency=2000.0, sla_compliance_rate=0.1,
        )
