...
from agent.baselines import ThresholdAgent, PredictiveHeuristicAgent
...
class AgentDeployer:
    ...
    def load_from_registry(self, company_name: str, agent_name: str):
        ...
        if not metadata:
            raise FileNotFoundError(
                f"Agent '{agent_name}' not found for company '{company_name}'"
            )
        ...
        if metadata.algorithm in ("dqn", "ppo"):
            if metadata.model_path and os.path.exists(metadata.model_path + ".zip"):
                ...
                logger.warning(
                    f"No trained model found at '{metadata.model_path}'. "
                    f"Falling back to threshold heuristic."
                )
        ...
        # Update status
        registry.update_agent_status(company_name, agent_name, "deployed",
                                     deployment_endpoint=self.ingress_url)
    ...
    def predict(self, telemetry: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.model is not None:
            ...
            return action_to_decision(
                int(action),
                f"[{self.algo.upper()}] Step {self.step_count}"
            )
    ...
    def post_decision(self, decision: Dict[str, Any]):
        ...
        resp = requests.post(
            f"{self.ingress_url}/decision", json=decision, headers=headers
        )
        resp.raise_for_status()
        return resp.json()
        ...
        try:
            while True:
                ...
                try:
                    ...
                    logger.error(
                        f"Cannot reach ingress at {self.ingress_url}. "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Deploy a trained Cloud Cost Optimizer agent to production"
    )
