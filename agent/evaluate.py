...
from env.environment import make_steady_env, make_spike_env, make_chaos_env
...
TASK_CONFIGS = {
    "steady": {
        ...
        "yaml_path": os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "tasks", "task_steady.yaml"),
    },
    "spike": {
        "make_env": make_spike_env,
        "yaml_path": os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "tasks", "task_spike.yaml"),
    },
    "chaos": {
        "make_env": make_chaos_env,
        "yaml_path": os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "tasks", "task_chaos.yaml"),
    },
}
    ...
    for task_name in tasks:
        ...
        for agent in agents:
            ...
            if verbose:
                ...
                print(f"    SLA: {grade['sla_score']:.3f}  Cost: {grade['cost_score']:.3f}  "
                      f"Latency: {grade['latency_score']:.3f}  Stability: {grade['stability_score']:.3f}")
                print(f"    Served: {ep.total_served:.0f}  Dropped: {ep.total_dropped:.0f}  "
...
if __name__ == "__main__":
    ...
    if args.model:
        ...
        trained_results = evaluate_trained_agent(
            args.model, algo=args.algo, seed=args.seed
        )
