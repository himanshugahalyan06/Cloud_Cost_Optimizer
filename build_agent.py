...
from agent.registry import AgentRegistry, AgentMetadata
    ...
    if algo in ("dqn", "ppo"):
        ...
        try:
            ...
            registry.update_agent_status(
                args.company, args.agent, "trained",
...
def cmd_evaluate(args):
    ...
    from agent.evaluate import evaluate_trained_agent, evaluate_baseline_agents
    from agent.baselines import ThresholdAgent, PredictiveHeuristicAgent, run_baseline
...
def cmd_list(args):
    ...
    if not agents:
        ...
        print(f"  {'─'*35} {'─'*12} {'─'*12} {'─'*20}")
        for a in agents:
            print(f"  {a['agent_id']:<35s} {a.get('algorithm','?'):<12s} "
